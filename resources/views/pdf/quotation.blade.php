...
<body>
    <div class='container'>
        ...
        <h3 style='text-align: center;'>{{  ?? 'Quotation' }} #{{ ->invoice_no }}</h3>

        <table class='details'>
            <tr>
                <th>Quotation No:</th><td>{{ ->invoice_no }}</td>
                <th>Date:</th><td>{{ ->created_at->format('d M Y') }}</td>
            </tr>
            <tr>
                <th>Customer Name:</th><td>{{ ->name ?? 'N/A' }}</td>
                <th>Customer Email:</th><td>{{ ->email ?? 'N/A' }}</td>
            </tr>
            <tr>
                <th>Customer Phone:</th><td>{{ ->phone ?? 'N/A' }}</td>
                <th>Customer Address:</th><td>{{ ->address ?? 'N/A' }}</td>
            </tr>
        </table>
        ...
        <table class='total'>
            <tr><td class='text-right'>Subtotal:</td><td>Rs {{ number_format(->total_amount - ->gst_amount, 2) }}</td></tr>
            <tr><td class='text-right'>GST Amount:</td><td>Rs {{ number_format(->gst_amount, 2) }}</td></tr>
